...
class ViewController: UIViewController, WKNavigationDelegate, UIDocumentInteractionControllerDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        StudyPal.webView.frame = calcWebviewFrame(webviewView: webviewView, toolbarView: nil)
    }
    
    @objc func keyboardWillHide(_ notification: NSNotification) {
        StudyPal.webView.setNeedsLayout()
    }
    
    func initWebView() {
        StudyPal.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(StudyPal.webView);
        
        StudyPal.webView.uiDelegate = self;
        
        StudyPal.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)

        if(pullToRefresh){
            let refreshControl = UIRefreshControl()
            refreshControl.addTarget(self, action: #selector(refreshWebView(_:)), for: UIControl.Event.valueChanged)
            StudyPal.webView.scrollView.addSubview(refreshControl)
            StudyPal.webView.scrollView.bounces = true
        }

        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = StudyPal.webView.observe(\.underPageBackgroundColor) { [unowned self] webView, _ in
                currentWebViewTheme = StudyPal.webView.underPageBackgroundColor.isLight() ?? true ? .light : .dark
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !StudyPal.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl() {
        StudyPal.webView.load(URLRequest(url: SceneDelegate.universalLinkToLaunch ?? SceneDelegate.shortcutLinkToLaunch ?? rootUrl))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            StudyPal.webView.isHidden = false
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                StudyPal.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(StudyPal.webView.estimatedProgress);
    ...
    deinit {
        StudyPal.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: StudyPal.webView)
        }
